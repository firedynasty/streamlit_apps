...
def _auth_token():
    ...
    return hashlib.sha256(pw.encode()).hexdigest()[:20]

def check_password():
    token = _auth_token()
    if st.session_state.get("authenticated") or st.query_params.get("auth") == token:
...
with col_right:
    if "pron_result" in st.session_state:
        ...
        st.markdown("#### Results")
        st.info("Record yourself reading the passage for instant feedback.")
