                ...
                for file in files_to_process:
                    ...
                    with tab1:
                        if generate_summary:
                            ...
                            # Create a status object that will auto-complete
                            status1 = st.status("Generating main analysis...")
                            result = coach.analyze_content(text, analysis_query)
                            # Mark as complete when done
                            status1.update(label="Main analysis complete", state="complete", expanded=True)
                    ...
                    with tab2:
                        status2 = st.status("Analyzing strengths & skills...")
                        coach.analyze_content(
                            text, "Extract and summarize key strengths, skills, and accomplishments"
                        )
                        status2.update(label="Strengths & skills analysis complete", state="complete", expanded=True)
                    
                    with tab3:
                        status3 = st.status("Identifying improvement areas...")
                        coach.analyze_content(text, "Identify areas for improvement and development")
                        status3.update(label="Improvement areas analysis complete", state="complete", expanded=True)
                        
                    with tab4:
                        status4 = st.status("Analyzing ATS optimization...")
                        coach.analyze_content(text, "Provide recommendations for ATS optimization and keyword alignment")
                        status4.update(label="ATS optimization analysis complete", state="complete", expanded=True)
